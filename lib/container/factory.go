...
func NewContainerHandler(factories Factories, name string, watchType watcher.ContainerWatchSource, metadataEnvAllowList []string, inHostNamespace bool) (ContainerHandler, bool, error) {
	...
	// can support it.
	for _, factory := range getReorderedFactoryList(factories, watchType) {
...
func getReorderedFactoryList(factories Factories, watchType watcher.ContainerWatchSource) []ContainerHandlerFactory {
	list := make([]ContainerHandlerFactory, 0, len(factories[watchType]))
