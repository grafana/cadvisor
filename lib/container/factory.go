...
// Factories maps a container watch source to the handler factories that
...
// InitializePlugins registers the given plugins and merges the container
// handler factories they return into a single Factories map, instead of each
// plugin registering itself into process-global state. This lets a caller
// (e.g. manager.New) run multiple independently configured instances in one
// process.
func InitializePlugins(factory info.MachineInfoFactory, plugins map[string]Plugin, fsInfo fs.FsInfo, includedMetrics MetricSet) Factories {
	allFactories := make(Factories)

	for name, plugin := range plugins {
		pluginFactories, err := plugin.Register(factory, fsInfo, includedMetrics)
		if err != nil {
		...
		for watchType, list := range pluginFactories {
...
func NewContainerHandler(factories Factories, name string, watchType watcher.ContainerWatchSource, metadataEnvAllowList []string, inHostNamespace bool) (ContainerHandler, bool, error) {
	...
	// can support it.
	for _, factory := range reorderedFactoryList(factories, watchType) {
...
// reorderedFactoryList returns factories[watchType] with the "raw" factory
// moved to the end, since it can accept a wide range of paths and should only
// be used once no more specific factory claims a container.
func reorderedFactoryList(factories Factories, watchType watcher.ContainerWatchSource) []ContainerHandlerFactory {
	list := factories[watchType]
	reordered := make([]ContainerHandlerFactory, 0, len(list))
	...
	if rawFactory != nil {
		reordered = append(reordered, rawFactory)
	}
