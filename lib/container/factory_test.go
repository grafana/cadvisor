...
func TestNewContainerHandler_FirstMatches(t *testing.T) {
	// Register one always yes factory.
...
func TestNewContainerHandler_SecondMatches(t *testing.T) {
	// Register one always no and one always yes factory.
...
func TestNewContainerHandler_NoneMatch(t *testing.T) {
	// Register two always no factories.
...
func TestRawContainerHandler_Last(t *testing.T) {
	...
	factories := container.Factories{watcher.Raw: []container.ContainerHandlerFactory{chf1, cfh2, cfh3}}

	cont, _, err := container.NewContainerHandler(factories, testContainerName, watcher.Raw, testMetadataEnvAllowList, true)
	// All factories report CanHandle=false by default, so creation fails; we only
	// care that raw would be evaluated last via getReorderedFactoryList.
	_ = cont
	_ = err

	// Build an accept-all raw factory after non-raw factories to verify ordering
	// indirectly: if raw were first and rejected, we'd never reach the accepting one.
	accepting := &mockContainerHandlerFactory{
		Name:           "crio",
		CanHandleValue: true,
		CanAcceptValue: true,
	}
	rawReject := &mockContainerHandlerFactory{
		Name:           "raw",
		CanHandleValue: true,
		CanAcceptValue: false,
	}
	factories = container.Factories{watcher.Raw: []container.ContainerHandlerFactory{rawReject, accepting}}
	mockContainer, err := mockFactory.NewContainerHandler(testContainerName, testMetadataEnvAllowList, true)
	if err != nil {
		t.Error(err)
	}
	accepting.On("NewContainerHandler", testContainerName).Return(mockContainer, nil)
	got, accept, err := container.NewContainerHandler(factories, testContainerName, watcher.Raw, testMetadataEnvAllowList, true)
	if err != nil || !accept || got == nil {
		t.Error("Expected non-raw factory to handle before raw when both can handle")
	}
}
